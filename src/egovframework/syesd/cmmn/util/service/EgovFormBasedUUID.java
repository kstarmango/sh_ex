...
public class EgovFormBasedUUID implements Serializable {
	...
	private static Logger logger = LogManager.getLogger(EgovFormBasedUUID.class);
    ...
    public static EgovFormBasedUUID nameUUIDFromBytes(byte[] name) {
        ...
        try {
        	...
        	logger.error("SHA-256 not supported");
        ...
        if (md == null) {
        	try {
				...
				logger.error("nameUUIDFromBytes NullPointerException error");
			}
        }
    ...
    public static EgovFormBasedUUID fromString(String name) {
        ...
        if (components.length != 5) {
        	try {			
				...
				logger.error("fromString NullPointerException error");
			}
        }
        for (int i = 0; i < 5; i++)
    ...
    public long timestamp() {
        if (version() != 1) {
        	try {			
				...
				logger.error("timestamp NullPointerException error");
			}
        }
    ...
    public int clockSequence() {
        if (version() != 1) {
        	try {			
				...
				logger.error("clockSequence NullPointerException error");
			}
        }
        if (sequence < 0) {
    ...
    public long node() {
        if (version() != 1) {
        	try {			
				...
				logger.error("node NullPointerException error");
			}
        }
        if (node < 0) {
