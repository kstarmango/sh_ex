...
/**
	...
	public static String addMinusChar(String date) {
	 ...
	 /**
	  * DB에서 자료 불러올때 줄바꿈이 안되면 사용
	  * @param rnStr 변경할 자료
	  * @return brStr 변경된 자료
	  */
	 public static String toBRLine( String rnStr ) {

		 String brStr = rnStr.replaceAll("\r\n", "<br>");
		 brStr = rnStr.replaceAll("\n", "<br>");
	  ...
	  * @return
	  */
	 public static String tagRemove(String html){
		 if(html == null){
			 return html;
		 }
		 //Pattern SCRIPTS = Pattern.compile("<(no)?script[^>]*>.*?</(no)?script>",Pattern.DOTALL);//스크립트 제거
		 //Pattern STYLE = Pattern.compile("<style[^>]*>.*</style>",Pattern.DOTALL); //스타일 제거
		 //Pattern TAGS = Pattern.compile("<(\"[^\"]*\"|\'[^\']*\'|[^\'\">])*>"); //태그 제거
		 //Pattern nTAGS = Pattern.compile("<\\w+\\s+[^<]*\\s*>"); //
		 //Pattern ENTITY_REFS = Pattern.compile("&[^;]+;"); //엔티티 제거
		 //Pattern WHITESPACE = Pattern.compile("\\s\\s+");  // 공백 제거
