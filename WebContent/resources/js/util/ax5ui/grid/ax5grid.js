        ...
        return function () {
            ...
            this.config = {
                ...
                tree: {
                    ...
                    icons: {
                        openedArrow: '▾',
                        collapsedArrow: '▸',
                        groupIcon: '<img src="/images/portal/common/icon_folder.png" align="absmiddle" />',
                        collapsedGroupIcon: '<img src="/images/portal/common/icon_folder.png" align="absmiddle" />',
                        itemIcon: '<img src="/images/portal/common/icon_folderIn.gif" align="absmiddle" />'
                    },
                    columnKeys: {
                        parentKey: "pid",
                        selfKey: "id",
            ...
            var alignGrid = function alignGrid(_isFirst) {
                ...
                if (!this.config.height) {
                    //if(this.$target.length>0 && this.$target[0].className == "gridWH"){
                    //    this.$["container"]["root"].css({ height: this.config._height = "100%" });
                    //}else
            ...
            this.setData = function (_data) {
                ...
                // 조회시 데이터변경 이벤트 Call(김철구)
                if(_data.list && _data.list.length > 0){
                    if (this.onDataChanged) {
                        var _self = this;
                        var _keys = Object.keys(_data.list[0]);

                        $.each(_keys, function(){
                            var key = this.toUpperCase();
                            if(key.indexOf("PLNT") > -1|| key.indexOf("PLANT") > -1){
                                var _key = this;
                                var _value = _data.list[this];
                                
                                _self.onDataChanged.call({
                                    self: _self,
                                    list: _self.list,
                                    dindex: 0,
                                    doindex: 0,
            ...
            this.exportExcel = function (_fileName) {
                ...
                console.log("header??",GRID.header.getExcelString.call(this))
                ...
                if (typeof _fileName === "undefined") {
                    ...
                    //GRID.excel.export.call(this, [table.join('')], _fileName); 기존
                	//20240729 수정 - 엑셀 다운로드 사용할 수 없는 파일 문제로 소스 변경
                	var tableHTML = table.join('');
                    var downloadLink;
        ...
        this.$["container"]["body"].on("click", '[data-ax5grid-column-attr]', function (e) {
            var panelName = void 0,
                ...
                "default": function _default(_column) {
                    ...
                    if (self.config.body.onFocusout) {
                        self.config.body.onFocusout.call(that, that, e);
                    }
                },
            ...
            var panelName = void 0,
                ...
                "default": function _default(_column) {
                    ...
                    if (U.isObject(editor)) {
                        //console.log("U.isObject(editor)", value);
                    ...
                    for (var _ri2 = 0, _rl2 = tblRowMaps.length; _ri2 < _rl2; _ri2++) {
                        for (var _ci2 = 0, _cl = tblRowMaps[_ri2].length; _ci2 < _cl; _ci2++) {
                            if (tblRowMaps[_ri2][_ci2].rowspan == 0) {
                                ...
                                if(tblRowMaps[_ri2][_ci2]["$"].attr("class").indexOf("grid-cell") > -1){
        ...
        active: function active(_focusedColumn, _e, _initValue) {
            ...
            for (var key in _focusedColumn) {
                ...
                if (!function (_editor, _type) {
                    ...
                    }else if(editor.type == "uploader") {
                        var newValue = void 0;
                        
                        console.log("GRID.data.setValue.call", _initValue);
                        GRID.data.setValue.call(self, dindex, doindex, col.key, newValue);
                    }
                    return this;
                }
...
(function () {
    ...
    var add = function add(_row, _dindex, _options) {
        ...
        if (this.config.tree.use) {
            ...
            this.list = list;
            // 로우 추가시 데이터변경 이벤트 Call(김철구)
           if (this.onDataChanged) {
                var _self = this;
                var _keys = Object.keys(_row);
               
                $.each(_keys, function(){
                    var key = this.toUpperCase();
                    if(key.indexOf("PLNT") > -1|| key.indexOf("PLANT") > -1){
                        var _key = this;
                        var _value = _row[this];
                        
                        _self.onDataChanged.call({
                            self: _self,
                            list: _self.list,
                            dindex: _row.__index,
                            doindex: _row.__original_index,
                            item: _self.list[_row.__index],
                            key: _key,
                            value: _value
                        });                            
                    }
...
(function () {
    ...
    var storeUploadedFiles = [];
    var uploader_evt_row;
    var edit_uploader = {
            useReturnToSave: true,
            editMode: "popup",
            _getHtml: function _getHtml(_root, _columnKey, _editor, _value) {
                // init 에서 사용하게 될 HTML 태그를 만들어 줍니다.
                return "<div data-ax5grid-editor=\"uploader\"></div>";
            },
            _bindUI: function _bindUI(_root, _columnKey, _$el, _editor, _$parent, _value) {
                var eConfig = {
                };
                ...
                // 새로운 엘리먼트 생성
                var $newDiv = jQuery("<div data-ax5grid-popup=\"uploader\" style=\"z-index: 9999;\">\n <div data-ax5uploader=\"upload\"> <div style=\"width:100%;height:" + (editorHeight - buttonHeight) + "px;\" class=\"form-control\" data-uploaded-box=\"upload\" data-ax5uploader-uploaded-box=\"inline\"></div>\n    <div style=\"height:" + buttonHeight + "px;padding:5px;text-align: right;\">\n  <button class=\"btn btn-default\" data-ax5uploader-button=\"clear\">초기화</button> <button class=\"btn btn-default\" data-ax5uploader-button=\"close\">닫기 </button> <button class=\"btn btn-default\" data-ax5uploader-button=\"selector\">파일선택 </button>\n  </div>\n</div>\n </div>");
                var $newUploader = $newDiv.find('[data-ax5uploader="upload"]'); 
                // 엘리먼트에 CSS 적용
                $newDiv.css({
                    position: "absolute",
                    left: offset.left,
                ...
                $newUploader.focus().select();

                var $newUploaderBox = $newUploader.find('[data-uploaded-box="upload"]');
                ...
                upload.setConfig({
                    ...
                    target: $newUploader,
                    form: eConfig.form,
                    multiple: eConfig.multiple,
                    manualUpload: eConfig.manualUpload,

                    progressBox: eConfig.progressBox,
                    progressBoxDirection: eConfig.progressBoxDirection,

                    dropZone: {
                        target: $newUploaderBox,
                        onclick: eConfig.dropZone.onclick,
                        ondragover: function () {
                    ...
                    uploadedBox: {
                        target: $newUploaderBox,
                        icon: eConfig.uploadedBox.icon,
                        columnKeys: eConfig.uploadedBox.columnKeys,
                        lang: eConfig.uploadedBox.lang,
                        onchange: eConfig.uploadedBox.onchange,
                        onclick: eConfig.uploadedBox.onclick
                    },

                    validateSelectedFiles: function () {
                        console.log("validateSelectedFiles",this);
                        return true;
                    },
                    onprogress: function () {

                    },
                    onuploaded: eConfig.onuploaded,
                ...
                var $newCloseBtn = $newUploader.find('[data-ax5uploader-button="close"]');
                var $newCleanBtn = $newUploader.find('[data-ax5uploader-button="clear"]');
                var returnValue;
                
                uploader_evt_row = "";
                $newCloseBtn.on("click", function(e) {
                    if(eConfig.uploadedBox.columnKeys.thumbnail || eConfig.uploadedBox.columnKeys.apiServerUrl){
                        returnValue = ($(this).parent().prev().find('img') ? $(this).parent().prev().find('img') : null);
                    }else{
                        $.each(upload.uploadedFiles, function(index, item){
                            returnValue = item.orgFileName;
                        });
                    }
                    //임시 파일 저장
                    storeUploadedFiles = upload.uploadedFiles;
                    
                    GRID.body.inlineEdit.deActive.call(_root, "RETURN", _columnKey, returnValue);
                    $newDiv.remove();
                    ax5.util.stopEvent(e.originalEvent);
                });

                $newCleanBtn.on("click", function(e) {
                    if(storeUploadedFiles){
                        var data = {
                                voList: (!storeUploadedFiles ? upload.uploadedFiles : storeUploadedFiles)
                        };
                        ...
                        gmes_ajaxLoad(
                                ...
                                ,function(data) {
                                    //그리드에 data set
                                    if(data.header.errorCode=="0000") { //성공
                                        gmes_dialog('success'
                                                , '파일 초기화'
                                                , '파일 리스트 초기화했습니다.'
                                                , 2000
                ...
                //기존 upload data 있으면 다시 넣기
                if(_value){
                    if(storeUploadedFiles){
                        upload.setUploadedFiles(storeUploadedFiles);
                    }
                }
    ...
    var makeFootSumTable = function makeFootSumTable(_footSumColumns) {
        ...
        for (var r = 0, rl = _footSumColumns.length; r < rl; r++) {
            ...
            for (var c = 0, cl = footSumRow.length; c < cl; c++) {
                ...
                var rowspan = footSumRow[c].rowspan || 1;
                if (footSumRow[c].label || footSumRow[c].key) {
                    table.rows[r].cols.push({
                        ...
                        customData: footSumRow[c].customData
                    });
                } else {
                    table.rows[r].cols.push({
                        colIndex: addC,
