				...
				success: function( data ) {
					...
					$('#load').hide();
					    ...
					    strPnu = data.pnu[0];
					  	...
					  	//infodetailshow(); //상세보기 바로 보여주기

					  	$('#info_mini_empty').empty();
					  	$('.hideList input').each(function() {
...
//										var maxY1 = now[1] + 20.0;	// top
							        	...
							        	//filter = ol.format.filter.within('the_geom', geometry, 'EPSG:4326')
							        	
							        } else {
										console.log("들어옴2 : ", geom);
										filter = ol.format.filter.contains('the_geom', new ol.geom.Point(geom), 'EPSG:3857')
										
							        	//filter = ol.format.filter.contains('the_geom', new ol.geom.Point([geom[1],geom[0]]), 'EPSG:4326'); //wfs호출 문제로 경위도 순서 변경
							        }
									...
									console.log("변환",new XMLSerializer().serializeToString(featureRequest))
...
//									console.log("layer_nm :" , layer_nm);
									...
									$.ajax({
										...
										data: {data :new XMLSerializer().serializeToString(featureRequest)},
										error : function(response, status, xhr){
											console.log("erreor",response)
					  	...
					  	infodetailshow(strHtml); //상세보기 바로 보여주기
		}
	});
}
...
function numberWithCommas(x) {
	...
	$(document).ready(function() {
		...
		$('#data_export').click(function() {
			console.log("데이터추출 클릭!")
			if( $('#export-mini').css("display") == "none"){
				$('#export-mini').show();
			} else {
				$('#export-mini').hide();
			}
...
//		measureClear();
	...
	function addInteraction(mearsureType, status) {
	  ...
	  //var type = (typeSelect.value == 'area' ? 'Polygon' : 'LineString');
	  	...
	  	if(type !== 'None'){
			...
			draw.on('drawend', function(evt) {
				...
				if(status != null || status != undefined){ //데이터 추출일 경우 
					coords = feature.getGeometry().getCoordinates();
					console.log("coords",coords)
					$('#area_sectDiv').css('display','block');
				}
	...
	var formatLength = function(line) {
	  	...
	  	length = Math.round(ol.sphere.getLength(line, {projection: geoMap.getView().getProjection()}) * 100) / 100;
	  	var output;
	  	if(length > 1000) {
	...
	var formatArea = function(polygon) {
		...
		//var area = 0;
	  	...
	  	var area = ol.sphere.getArea(polygon, {projection: geoMap.getView().getProjection()});
...
//			now_layersCity1.setOpacity(parseFloat(0.5));
	...
	function area_sect() {
		...
		addInteraction(typeSelect,'sect');
	}
	...
	//추출유형 별 영역 활성화
	function searchTypeLayout(_type) {
		$("#geom_searchType_ul li").removeClass("on"); //on 제거
		
		$("#smLi_"+_type).addClass("on"); //클릭한 type on추가
		
		if(_type == "intersects") $('#area_intersects').show();
		else $('#area_intersects').hide();
		
		$('#exportType').val(_type)
	}
	
	//데이터 추출하기
	function fn_exportArea(){
		 var result;
		if(exportLayerList.length != 0 ){
			result = confirm('현재 지적 외 레이어 '+exportLayerList.length+'개가 선택되었습니다. 데이터 추출을 진행하시겠습니까?');
		}else{
			result = confirm('현재 지적도를 기준으로 데이터 추출을 진행하시겠습니까?');
		}
	        if(result) {
	           //yes
	        	if(!exportLayerList.includes("landsys_gis.ctnu_lgstr")) exportLayerList.push("landsys_gis.ctnu_lgstr")
	        	//$('#load').show();
	        	
	        	var geom = "POLYGON((";
	    		for(i=0; i<coords[0].length; i++){
	    			var cor = coords[0][i];
	    			geom += cor.toString().replace(",", " ") + ",";
	    		}
	    		...
	    		var param = {
	    				format : "SHP",
	    				layerList : exportLayerList.toString(), //데이터 추출 레이어 목록 - 레이어스키마.레이어 테이블명으로 구성
	    				geom : geom,
	    				type: $('#exportType').val(), 	//추출유형
	    				areaPer : $('#areaPer').val()	//포함면적
	    		}
	    		
	    		var formObj = $("#dataExportForm");

	    		$('#format').val("SHP");
	    		$('#layerList').val(exportLayerList.toString());
	    		$('#geom').val(geom);
	    		
	    		formObj.attr("action", "/web/cmmn/dataExport.do");
	...
	function menuToogle(_this){
		$('.BtnWrapSM button').removeClass("hover");
