...
var geostats = function(a) {
  ...
  this.decimalFormat = function(a) {
    ...
    for (var i = 0; i < a.length; i++) {
      ...
      if (isNumber(a[i])) {
        b[i] = parseFloat(Number(a[i]).toFixed(this.precision));
      } else {
        b[i] = a[i];
      }
    }
   ...
   * order :  null, 'ASC', 'DESC'
   */
  this.getHtmlLegend = function(legendTitle, colors, legend, counter, callback, mode, order) {
    ...
    if(this.is_uniqueValues == false) {
      ...
      for (i = 0; i < (this.ranges.length); i++) {
        ...
        el = '<span class="legendSpan">' + el + '</span>';
    ...
    var content  = '<div class="geostats-legend"><div class="geostats-legend-title legendTitle">' + legendTitle + '</div>';
