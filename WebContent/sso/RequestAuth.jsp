...
<%
	...
	try {
		...
		result = sp.generateAuthnRequest(request, request.getParameter(PARAM_LOGIN_ID), request.getParameter(PARAM_LOGIN_PW));

		if (Integer.parseInt((String) result.get("code")) != MStatus.SUCCESS) {
			if (Integer.parseInt((String) result.get("code")) == MStatus.IS_FIRST_LOGIN) {
				System.out.println("codecode :::: firstLogin" );
				Util.sendinitPwURL(response, PW_INIT_RELAYSTATE, String.valueOf(MStatus.IS_FIRST_LOGIN), "is first login ", request.getParameter(PARAM_LOGIN_ID));
				return;
			}
