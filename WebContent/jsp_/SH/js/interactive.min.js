...
function interactive(e, t) {
    ...
    n.classList.add("interactive"), n.style.zIndex = "inherit", void 0 === t ? (resizable(n), draggable(n), close(n), minMax(n)) : (!1 !== t.resize && resizable(n), !1 !== t.drag && draggable(n), !1 !== t.close && close(n), !1 !== t.minMax && minMax(n, t.minZone, t.minMaxIcons, t.minDoubleClick)), n.onmousedown = changeStackOrder
}
...
function initialResizeCssProperties(e, t) {
    let n = getComputedStyle(e), i = n.getPropertyValue("width"), o = n.getPropertyValue("height");
    "0px" == i && (i = "200px"), "0px" == o && (o = "150px"), t.style.top = n.getPropertyValue("top"), t.style.left = n.getPropertyValue("left"), t.style.gridTemplateRows = "3px " + o + " 3px", t.style.gridTemplateColumns = "3px " + i + " 3px", t.style.backgroundColor = n.getPropertyValue("background-color"), e.style.top = "0px", e.style.left = "0px", e.style.width = i, e.style.height = o
}
function addResizePointFunctionality(e, t, n) {
    n.onmousedown = function() {
        1 == event.which && trackMouseDragPlusAction({
            action: "resize",
            param: [e, t, n.className]
        })
    }
}
...
function changeHorizontalMeasures(e, t, n, i) {
    if (void 0 === i) return;
    1 == i && (n = -n);
    let o = parseInt(e.style.width.slice(0, - 2)) + n, l = t.offsetLeft;
...
function changeVerticalMeasures(e, t, n, i) {
    if (void 0 === i) return;
    3 == i && (n = -n);
    let o = parseInt(e.style.height.slice(0, - 2)) + n, l = t.offsetTop;
...
function getDragNewPosition(e, t) {
    let n = getElementOffsetAndMeasures(e), i = {
        x: n.left - t.x,
        y: n.top - t.y
    };
    return preventDragOutsideScreen(n, i, {
        left: i.x,
        top: i.y,
        right: i.x + n.width,
        bottom: i.y + n.height
...
function close(e) {
    addFunctionButton(e, [{
        name: "close",
        path: "M45.1,41.6l-3.3,3.3L5.1,8.2l3.3-3.3L45.1,41.6z M8.4,45.1l-3.3-3.3L41.8,5.1l3.3,3.3L8.4,45.1z"
    }]), addCloseFunctionality(e)
}
function addCloseFunctionality(e) {
    closeBtn = getButton(e, "closeBtn"), closeBtn.onclick = function() {
        let t = e.parentNode;
...
function minMax(e, t, n, i) {
    if (!1 !== n) {
        addFunctionButton(e, [{
            name: "max",
            path: "M27.3,45h-4.7V5h4.7V45z M5,27.3v-4.6H45v4.6H5z"
        }, {
            name: "min",
            path: "M5,27.3v-4.6H45v4.6H5z"
        }])
    }
...
function addFunctionButton(e, t) {
    ...
    for (let e = 0, o = t.length; e < o; e++) {
        let o = createElementWithClassName("button", t[e].name + "Btn mmcBtn"), l = createSvgShape({
            svg: [{
                attr: "class",
                value: "svgIcon"
            }, {
...
let count, elementWidth, dropdown, numItems, minStorage = [];
...
function getItemCountToFitElementByWidth(e, t) {
    if (null != e) {
        let n = t.clientWidth, i = window.getComputedStyle(e), o = parseInt(i.getPropertyValue("width").slice(0, - 2)), l = parseInt(i.getPropertyValue("margin-left").slice(0, - 2)), a = parseInt(i.getPropertyValue("margin-right").slice(0, - 2));
        return elementWidth = o + l + a, Math.floor(n / elementWidth)
    }
}
function createMinimizedElementRep(e, t) {
    let n = createElementWithIdAndClassName("span", e, "minimizedItem"), i = createElementWithClassName("p", "minimizedTitle");
...
function minimizeUI(e) {
    ...
    if (void 0 === dropdown) if (numItems = getItemCountToFitElementByWidth(t.firstElementChild, t), minStorage.length > numItems) horizontalRepToDropdownList(numItems, t), dropdown = !0;
    else {
...
function maximize() {
    ...
    let e = parseInt(this.id), t = document.getElementById(minStorage[e].id);
...
function fromDropdownToHorizontalMinimized(e) {
    let t = e.previousSibling, n = e.parentElement;
    n.removeChild(t), n.removeChild(e), dropdown = void 0;
    let i = 0;
    for (let e = 0, t = minStorage.length; e < t; e++) "" != minStorage[e] && (minStorage[i] = {
        id: minStorage[e].id,
        title: minStorage[e].title
    }, i++);
...
function addDropdown(e) {
    ...
    return n.id = "dropdownList", e.appendChild(t), e.appendChild(n), t.onclick = function() {
...
function addFullScreenMaximizeFunction(e) {
    ...
    null != t && null != t && (t.onclick = function() {
        let t, n = "" + e.id;
        if (e.classList.contains("resizable") && (t = !0), void 0 === maxStorage[n]) {
            let n = window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth, i = window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight, o = e.id;
            if (t) {
                let t = e.parentNode;
                maxStorage[o] = {
                    actualSize: getElementSizeAndPosition(t)
                }, e.style.width = "100%", e.style.height = "100%", t.style.top = "0px", t.style.left = "0px", t.style.margin = "0px", t.style.gridTemplateRows = "3px " + (i - 6) + "px 3px", t.style.gridTemplateColumns = "3px " + (n - 6) + "px 3px"
            } else maxStorage[o] = {
                actualSize: getElementSizeAndPosition(e)
            }, e.style.top = "0px", e.style.left = "0px", e.style.margin = "0px", e.style.width = n + "px", e.style.height = i + "px"
        } else {
            if (t) {
                let t = e.parentElement;
                t.style.top = maxStorage[n].actualSize.top, t.style.left = maxStorage[n].actualSize.left, t.style.margin = maxStorage[n].actualSize.margin, t.style.gridTemplateRows = maxStorage[n].actualSize.gridRow, t.style.gridTemplateColumns = maxStorage[n].actualSize.gridCol
            } else e.style.top = maxStorage[n].actualSize.top, e.style.left = maxStorage[n].actualSize.left, e.style.width = maxStorage[n].actualSize.width, e.style.margin = maxStorage[n].actualSize.margin, e.style.height = maxStorage[n].actualSize.height;
            delete maxStorage[n]
        }
    })
}
...
function changeStackOrder() {
    ...
    for (let t = 0, n = e.length; t < n; t++) e[t] == this ? e[t].classList.contains("resizable") ? e[t].parentElement.style.zIndex = 2 : e[t].style.zIndex = 2 : e[t].classList.contains("resizable") ? e[t].parentElement.style.zIndex = 1 : e[t].style.zIndex = "inherit"
}
...
function resizeOnWindowChange() {
    ...
    for (let t = 0, n = e.length; t < n; t++) {
        let n = document.body.clientWidth, i = document.body.clientHeight, o = e[t].firstElementChild, l = e[t], a = l.offsetLeft;
...
function trackMouseDragPlusAction(e) {
    let t = event.clientX, n = event.clientY;
    document.onmouseup = dragMouseStop, document.onmousemove = function() {
        let i = event.clientX, o = event.clientY;
        dragAction(e, {
            x: t - i,
            y: n - o
        }), t = i, n = o
