...
// DECLARE FUNCTIONS THAT WILL BE EXPOSED THROUGH PROTOTYPE 
        ...
        // "translate" the leaflet graphics item to something compatible with the esri format before adding
        var addLeafLetGraphics = function (leafletgraphics) {
            if (leafletgraphics instanceof L.FeatureGroup) {
                for (var i in leafletgraphics._layers) {
                    var geometry = leafletgraphics._layers[i];
                    var quasiEsriGraphic = {
                        geometry: {}
                    };
                    if (geometry instanceof L.Marker) {
                        quasiEsriGraphic.geometry.type = "POINT";
                        quasiEsriGraphic.geometry.x = geometry.getLatLng().lng;
                        quasiEsriGraphic.geometry.y = geometry.getLatLng().lat;
                        this._pointgraphics.push(quasiEsriGraphic);
                    }
                    if (geometry instanceof L.Polyline) {
                        ...
                        if (geometry instanceof L.Polygon) {
                            ...
                            path.push([lastLatLng.lng, lastLatLng.lat]);
                            quasiEsriGraphic.geometry.rings.push(path);
                            this._polygongraphics.push(quasiEsriGraphic);
                        }
                        else {
                            quasiEsriGraphic.geometry.type = "POLYLINE";
...
// DEFINE THE OBJECT THAT WILL REPRESENT THE PROTOTYPE
        ...
        return {
            ...
            addLeafLetGraphics: function () {
                return addLeafLetGraphics.call(this, arguments[0]);
            },
