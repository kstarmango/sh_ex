...
<head>
	...
	$(document).ready(function() {
		...
		$("#contents tbody tr td").click(function(e) {
            if($(this).attr('p-auth-no') == '' || $(this).attr('p-auth-no') == undefined) 
            	return
            
            // 초기화
            p_auth_no = $(this).attr('p-auth-no');
   			        ...
   			        if(data.result == "Y" && data.progrmInfo.length > 0) {
   			         	for (i = 0; i < data.progrmInfo.length; i++) {	
		   			       var row = 	"  	<tr>                                                                     	 " +
							            ...
							            "                    <input type='checkbox' id='t_progrms' name='t_progrms' value='" + data.progrmInfo[i].progrm_no + "'" + (data.progrmInfo[i].view_yn == 'Y' ? 'checked' : '') + " /> " +
										...
										<c:when test="${!empty progrmInfoList}">
											<c:forEach items="${progrmInfoList}" var="result" varStatus="status">
												<tr onclick="javascript:fn_link_detail('${result.p_auth_no}'); return false;" data-toggle="modal"  data-target="#progrmDetail">
			                                        ...
			                                        <td p-auth-no='${result.p_auth_no}' title='메뉴 목록을 확인 할 수 있습니다.'>
			                                       		<a href='#progrmAuthList' data-toggle="modal"> + 메뉴 목록 </a>
			                                        </td>			                                        
			                                    </tr>
											</c:forEach>
										</c:when>
	                        ...
	                        <table class="table table-custom table-cen table-num text-center table-hover" width="100%">
	                            ...
	                            <tbody>
									...
									<tr>
									    ...
									    <td>
											<select name="admin_yn" id="admin_yn" class="form-control col-md-3 input-sm">
												<option value=""></option>
	                        ...
	                        <table class="table table-custom table-cen table-num text-center table-hover" width="100%">
	                            ...
	                            <tbody>
									...
									<tr>
										<th scope="row">관리자여부</th>
										<td colspan="3">
											<select name="new_admin_yn" id="new_admin_yn" class="form-control col-md-3 input-sm">
												<option value=""></option>
									        	<option value='Y'>예</option>
	            ...
	            <div class="col-sm-12">
	                <div class="card-box big-card-box last table-responsive searchResult">
	                    ...
	                    <div class="table-wrap m-t-30" style='overflow-y:auto; height:400px;'>
	                        <table class="table table-custom table-cen table-num text-center table-hover" id='progrmAuthRow' width="100%">
	                            <colgroup>
	                                <col width="5%">
	                                <col width="45%">
	                                <col width="25%">
		                        ...
		                        <thead>
			                        <tr>
			                            ...
			                            <th>메뉴명</th>
			                            <th>관리자 여부</th>
			                            <th>권한부여 여부 <input name="toggle_auth" id="toggle_auth" type="checkbox" /></th>
			                        </tr>
		                        </thead>
		                        <tbody>
		                        </tbody>
