...
$(document).ready(function(){
	...
	$("#selsggListAl").change(function() {
		...
		if($("#search").attr("edit") == "Y"){
			var nm =  $("#search").val().split(" "); //주소 상세히 입력시 가능

			var endsWithGu = nm.filter(function(value) { // "구"로 끝나는 각 값들을 찾습니다.
			    return value.endsWith("구");
			});

			if(endsWithGu.length == 0){
				//alert("주소를 상세히 입력해 주시기 바랍니다."); 
				$("#search").attr("edit","N");
				
				return false;
			}
			// endsWithGu 값과 동일한 값을 가진 옵션을 찾습니다.
			$("#selsggListAl").find("option").each(function() {
			    if ($(this).text() === endsWithGu[0]) {
			        $(this).prop('selected', 'selected');
			        $("#selsggListAl").val($(this).val());
			    }
			});
		}
	...
	$("#selEmdListAl").change(function() {	
		...
		if($("#search").attr("edit") == "Y"){ 
			var nm =  $("#search").val().split(" "); //주소 상세히 입력시 가능
			var endsWithDong = nm.filter(function(value) { // "동"로 끝나는 각 값들을 찾습니다.
			    return value.endsWith("동");
			});

			if(endsWithDong.length == 0){
				//alert("주소를 상세히 입력해 주시기 바랍니다."); 
				$("#search").attr("edit","N");
				return false;
			}
			
			var endsWithJibun = nm.filter(function(value) { // 지번찾기
				 return !isNaN(value[value.length - 1]);
			});
			
			// 배열 nm에 '산'이라는 한 글자가 있는지 확인 true,false
			var endsWithSan = nm.some(function(value) {
			    return value.includes('산') && value.length === 1; // '산'이 포함되고 길이가 1인 경우 true 반환
			});

			if (endsWithSan) {
				$("input:checkbox[id='san']").prop('checked', true); 
			}else $("input:checkbox[id='san']").prop('checked', false); 

			// endsWithDong 값과 동일한 값을 가진 옵션을 찾습니다.
			 $("#selEmdListAl").find("option").each(function() {
			    if ($(this).text() === endsWithDong[0]) {
			        $(this).prop('selected', 'selected');
			        $("#selEmdListAl").val($(this).val());
		            
			    	 $("#jibun").val(endsWithJibun);  //지번입력
			    }
			});
		}
		...
		if($("#search").attr("edit") == "Y"){
			addr_search_jibun(1);  //도로명검색결과없었을때 지번검색하기
			$("#search").attr("edit","N");  //검색종료
		}
	});
