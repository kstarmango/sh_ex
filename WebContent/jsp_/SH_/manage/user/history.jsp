...
<!--     <script src="/jsp/SH/js/jquery.validate.min.js"></script> -->
	...
	function fn_link_acUser(pageNo) {
		$("#pageIndex").val(pageNo);
		 ...
		 $("form").attr("action","memAccessed.do");
		 $("form").submit();
	}
	        ...
	        <div class="row">
	            <div class="col-sm-12">
	                <div class="page-title-box">
	                    ...
	                    <h4 class="page-title">사용자 접속현황</h4>
	                </div>
	            </div>
	        </div>
	        <!-- End Page-Title -->
                                	...
                                	<c:choose>
										<c:when test="${!empty userHistList}">
											<c:forEach items="${userHistList}" var="result" varStatus="status">
												<tr>
													<td>
													<c:out value="${view01Cnt.totalRecordCount - (result.rno - 1)}" escapeXml="false" /></td>
			                                        ...
			                                        <td>
			                                           <c:choose>
			                                             <c:when test="${result.in_out eq 'in'}">
			                                                                                                     로그인
			                                             </c:when>
			                                             <c:otherwise>
			                                                  	로그아웃
