...
function exportLeaflet(){
	var isLand = $('input:radio[id=export_land]').is(':checked');
	if(isLand)
		exportLand()
	else
		exportBuild()
...
function exportLand() {
	...
	input1.setAttribute("name", "pk");
	input1.setAttribute("value", b);

	var input2 = document.createElement('input');
	input2.setAttribute("type", "hidden");
	input2.setAttribute("name", "kind");
	input2.setAttribute("value", "land");

	newForm.name   = 'newForm';
	newForm.target = '_blank';
	newForm.method = 'POST';
	newForm.action = '/ajaxDB_data_list.do';
		                ...
		                if (outputObject[shapefiletype]['successful']) {
		                    ...
		                    for (actualfile in outputObject[shapefiletype]['shapefile']) {
		                        if (outputObject[shapefiletype]['shapefile'].hasOwnProperty(actualfile)) {
		                            saver.addData({
		                                filename: shapefiletype + "_shapefile_land",
...
function exportBuild() {
	...
	input2.setAttribute("type", "hidden");
	input2.setAttribute("name", "kind");
	input2.setAttribute("value", "build");
		...
		success: function( data ) {
			...
			for(i=0; i<data.pnu.length; i++){
				...
				{	geometry:
					...
					{
						pnu: 				data.pnu[i]+"",
						//bild_prpos_nm: 		data.bild_prpos_nm[i]+"",
						bild_strct_code: 	data.bild_strct_code[i]+"",
						//bild_strct_nm: 		data.bild_strct_nm[i]+"",
						bild_ar: 			data.bild_ar[i]+"",
						totar: 				data.totar[i]+""
					}
				};
				graphicArray.push(graphic);
		            ...
		            if (outputObject.hasOwnProperty(shapefiletype)) {
		                if (outputObject[shapefiletype]['successful']) {
		                    ...
		                    for (actualfile in outputObject[shapefiletype]['shapefile']) {
		                        if (outputObject[shapefiletype]['shapefile'].hasOwnProperty(actualfile)) {
		                            saver.addData({
		                                filename: shapefiletype + "_shapefile_build",
		                                extension: actualfile,
		                                datablob: outputObject[shapefiletype]['shapefile'][actualfile]
		                            });
		                        }
...
//		         	saveAs(saver.data[2]['datablob'], saver.data[2]['filename']+"."+saver.data[2]['extension']);
    	...
    	<!-- 데이터추출 Side-Panel  "tab-pane fade toptab" role="tabpanel"-->
		<div class="mapWrap" id="export-mini" style="width: 900px; height:600px; top: 57px; display: none; background: white;">
...
<!-- 	                                <h5 class="list-group-item-heading">준비중입니다.</h5> -->
									<div class="form-group row">
										...
										<div class="col-xs-6">
											<input type="radio" id="export_buld" name="exports" value="02" />&nbsp;
